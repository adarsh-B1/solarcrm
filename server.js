...
// ── Receive customers sent from portal bookmarklet ──
app.post('/api/sync-portal-customers', auth, async (req, res) => {
  if (req.vendor.plan === 'expired') return res.status(403).json({ error: 'SUBSCRIPTION_EXPIRED' });
  
  const portalCustomers = req.body.customers;
  if (!Array.isArray(portalCustomers) || !portalCustomers.length) {
    return res.status(400).json({ error: 'No customers provided' });
  }

  try {
    const { data: vendor } = await supabase.from('vendors').select('company').eq('id', req.vendor.id).single();
    const prefix = (vendor?.company || 'SOL').slice(0, 3).toUpperCase();
    
    const { data: existing } = await supabase.from('customers').select('*').eq('vendor_id', req.vendor.id);
...
/g, ' ').trim();
      ...
      const consumerAcc = String(p.consumerNumber||p.consumer_number||p.caNumber||p.consumerNo||p.ca_number||'');
      const appNo = String(p.applicationNumber||p.application_number||p.appNo||p.applicationNo||'');
      
      // Find existing customer to preserve ID
      const existingC = existingList.find(ec =>
        (consumerAcc && ec.consumer_acc === consumerAcc) ||
        (appNo && ec.app_no === appNo) ||
        (appNo && ec.notes && ec.notes.includes(appNo))
      );
      
      counter++;
      const id = existingC ? existingC.id : prefix + '-' + String(counter).padStart(3, '0');

      return {
        id, vendor_id: req.vendor.id,
        name: String(p.consumerName||p.consumer_name||p.applicantName||p.name||'Unknown').trim(),
        phone: String(p.mobileNumber||p.mobile||p.mobileNo||p.phone||'').replace(/\D/g,'').slice(-10),
        kw: parseFloat(p.proposedCapacity||p.capacity||p.proposedCapacityKwp||0) || 0,
        consumer_acc: consumerAcc,
        app_no: appNo,
        discom: String(p.discomName||p.discom||p.discom_name||''),
        submitted_on: String(p.submittedOn||p.applicationDate||p.createdDate||''),
        status_raw: statusRaw,
        loc: String(p.district||p.city||p.stateName||''),
        sanctioned_load: parseFloat(p.sanctionedLoad||p.sanctioned_load||0) || 0,
        agr, fin, dcr, inst, insp,
        sub_req: subReq, sub_disb: subDisb, sub: subDisb,
        sub_amt: 0, sub_recv: 0, loan: 0,
        notes: [appNo?'App No: '+appNo:'', p.discomName?'Discom: '+p.discomName:''].filter(Boolean).join(' | '),
        updated_at: new Date()
      };
    });
    ...
    const { error } = await supabase.from('customers').upsert(rows, { onConflict: 'id,vendor_id' });
    if (error) { console.error('Upsert error:', error); return res.status(500).json({ error: error.message }); }

    // Update portal_token_updated_at to track last sync
    await supabase.from('vendors').update({ portal_token_updated_at: new Date() }).eq('id', req.vendor.id);

    console.log(`[Sync] Vendor ${req.vendor.id}: ${rows.length} customers saved from bookmarklet`);
    res.json({ success: true, count: rows.length });

  } catch(e) {
    console.error('sync-portal-customers error:', e);
